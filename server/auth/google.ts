...
export class AuthGoogle implements AuthStrategy {
  private client = new OAuth2Client(options);
  ...
  async getUserProfile(token: string): Promise<StrategyUserProfile | null> {
    const verifyOpts: VerifyIdTokenOptions = {
      idToken: token,
      audience: process.env.GOOGLE_CLIENT_ID,
    };

    const ticket = await this.client.verifyIdToken(verifyOpts);
  ...
  async refreshToken(refreshToken: string): Promise<string | null> {
    try {
      ...
      throw new Error(`Failed to refresh token from Google: ${error}`);
    }
  }
